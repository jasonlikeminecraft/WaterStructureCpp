...
class QingXuStructure final : public IStructure {
public:
    explicit QingXuStructure(RuntimeRegistry& registry) : mRegistry(registry), mStore(registry) {}

    StructureId id() const noexcept override { return StructureId::QingXuV1; }
    std::string_view name() const noexcept override { return "QingXuV1"; }
...
private:
    std::uint32_t runtime_id(std::string name);
