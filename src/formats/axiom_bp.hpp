...
class AxiomBpReader final : public IStructure {
public:
    explicit AxiomBpReader(RuntimeRegistry& registry) : mRegistry(registry), mStore(registry) {}

    StructureId id() const noexcept override { return StructureId::AxiomBP; }
    std::string_view name() const noexcept override { return "AxiomBP"; }
