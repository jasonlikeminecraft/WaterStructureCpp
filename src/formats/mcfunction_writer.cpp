    ...
    const ConversionOptions& options)
{
    ...
    // Clearing the complete box preserves both air holes and trailing-air
    // dimensions. Callers that are merging into an existing world can opt out
    // and receive only non-air placement commands.
    if (options.clear_air) {
