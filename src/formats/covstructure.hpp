...
class CovStructureReader final : public IStructure {
public:
    explicit CovStructureReader(RuntimeRegistry& registry) : mRegistry(registry), mStore(registry) {}
    StructureId id() const noexcept override { return StructureId::CovStructure; }
    std::string_view name() const noexcept override { return "CovStructure"; }
