...
std::array<std::uint8_t, 16> md5(std::span<const std::uint8_t> input)
{
    ...
    if (BCryptOpenAlgorithmProvider(&algorithm, BCRYPT_MD5_ALGORITHM, nullptr, 0) < 0) {
        throw std::runtime_error("cannot open Windows MD5 provider for TIBI");
    }
    struct Guard {
    ...
    if (BCryptGetProperty(algorithm, BCRYPT_OBJECT_LENGTH,
        ...
        throw std::runtime_error("cannot query Windows MD5 object size for TIBI");
    }
    std::vector<std::uint8_t> object(object_size);
    if (BCryptCreateHash(algorithm, &hash, object.data(), object_size, nullptr, 0, 0) < 0) {
        throw std::runtime_error("cannot create Windows MD5 hash for TIBI");
    }
    if (input.size() > std::numeric_limits<ULONG>::max() ||
        ...
        throw std::runtime_error("cannot hash TIBI XOR key material");
    }
    std::array<std::uint8_t, 16> result{};
    if (BCryptFinishHash(hash, result.data(), static_cast<ULONG>(result.size()), 0) < 0) {
        throw std::runtime_error("cannot finish TIBI XOR key hash");
    }
    return result;
}
