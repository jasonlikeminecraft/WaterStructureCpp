...
value& value::operator=(std::string&& str)
{
    if(!tag_)
        ...
        tag_->as<tag_string>().set(std::move(str));
    return *this;
}

value::operator const std::string&() const
{
    return tag_->as<tag_string>().get();
}

value& value::at(const std::string& key)
{
    return tag_->as<tag_compound>().at(key);
}
...
{
    return tag_->as<tag_compound>()[key];
}
